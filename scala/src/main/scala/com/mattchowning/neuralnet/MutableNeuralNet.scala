...
object MutableNeuralNet extends NeuralNetFactory {

  // FIXME why isn't the override here required?
  override def withWeights(weights       : List[DenseMatrix[Double]],
                           activationFunc: ActivationFunc,
                           costFunc      : CostFunc,
                           learningRate  : Double) = {
    new MutableNeuralNet(weights, activationFunc, costFunc, learningRate)
  }
}
